...
\begin{quote}
  \emph{Use the same principle as with toilet rolls: 
...
The filled bag is transported to the deposit containers in the parking garage 
next to the deposit tank and closed with a zip tie.

During the bar shift while it is quiet or during cleaning, we go for a walk around Nygaard
and empty the deposit boxes, and put new bags in if they are full.
An overview of where the deposit boxes are can be found \href{https://media.fredagscafeen.dk/guides/pantspande.pdf}{here}.

\subsection{Other}
\label{sec:intra:other}

\begin{itemize}
    \item Bartenders get, in addition to dinner, one beer/cider/RTD of their choice for a maximum of 50 kr.,
    one bag of chips, and \textit{unlimited} free soda during their bar shift.

    \item We rent out various board games/dice/playing cards/ping pong balls/pool.
    ...
    \item It is possible to play music during the bar shift, by using the
    ...
    \item If a customer is considering purchasing a specialty cask or cider, it can be a good idea 
    to offer them a taste so they know what they are getting. This often converts into a sale, but 
    also helps to provide a good experience for customers.

    \item Remember to clear the tables of trash and empty bottles along the way so that new customers can come.

    \item The bell is usually rung \textit{last round}\index{Last round}
    about fifteen minutes before the bar closes
    (i.e. at 9:45 PM). There is a large and beautiful bell that the lucky bartender
    is allowed to ring.

    \item At 10:00 PM we play ``sod off''-music and no more sales are made.
\end{itemize}
